...
export default function LoginPage() {
  ...
  const handleInstagramLogin = async () => {
    // Instagram login goes through Facebook OAuth with Instagram permissions
    await supabase.auth.signInWithOAuth({
      provider: 'facebook',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?provider=instagram`,
        ...
        <div className="glass-card p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-flow-gray-400 text-sm">Sign in to join the community</p>
            </div>
          </div>
          ...
          {sent ? (
            ...
            <>
              ...
              <div className="space-y-3 mb-6">
                {/* Instagram — primary CTA for artists */}
                <button onClick={handleInstagramLogin} className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl font-medium text-sm border-2 border-transparent bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F77737] text-white hover:opacity-90 transition-opacity">
                  <svg className="w-5 h-5" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="6" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="12" cy="12" r="5" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="18" cy="6" r="1.5" fill="currentColor"/></svg>
